...
const EditTask = () => {
  ...
  const validateForm = () => {
    ...
    // Status
    if (!['pending', 'in_progress', 'completed', 'approved', 'rejected'].includes(formData.status)) {
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      ...
      case 'status':
        if (!['pending', 'in_progress', 'completed', 'approved', 'rejected'].includes(value)) fieldErrors.status = 'Invalid status selected';
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white dark:bg-gray-800 shadow-xl rounded-xl overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-indigo-600 to-blue-800 px-6 py-8">
            ...
            <p className="text-indigo-100">Update task details, assignment, and priority</p>
          </div>
